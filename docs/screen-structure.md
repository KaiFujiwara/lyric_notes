...
- カスタム編集UI: 設定画面またはプロジェクト詳細のメニューに「ジャンルテンプレ編集」を配置。リスト＋編集モーダルで name/description/sections(orderIndex, defaultLineCount) を編集。空状態時は「まだジャンルテンプレはありません」「マスタから複製」CTAを表示。
